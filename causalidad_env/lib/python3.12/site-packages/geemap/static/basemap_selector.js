...
var BasemapSelector = class extends LitWidget {
  constructor() {
    super(...arguments);
    this.basemaps = {};
    this.provider = "";
...
__decorateClass([
  n4({ type: Object })
], BasemapSelector.prototype, "basemaps", 2);
__decorateClass([
  n4({ type: String })
], BasemapSelector.prototype, "provider", 2);
__decorateClass([
  n4({ type: String })
], BasemapSelector.prototype, "resource", 2);
...
function render({ model, el }) {
  loadFonts();
  const row = document.createElement(BasemapSelector.componentName);
  row.model = model;
  el.appendChild(row);
}
var basemap_selector_default = { render };
export {
  BasemapSelector,
  basemap_selector_default as default
};
