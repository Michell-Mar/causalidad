...
var ee = "important";
var ie = " !" + ee;
var se = e4(class extends i3 {
  constructor(e5) {
    if (super(e5), e5.type !== t2.ATTRIBUTE || "style" !== e5.name || e5.strings?.length > 2) throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.");
  }
  render(t3) {
    return Object.keys(t3).reduce(((e5, r4) => {
      const s2 = t3[r4];
      return null == s2 ? e5 : e5 + `${r4 = r4.includes("-") ? r4 : r4.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g, "-$&").toLowerCase()}:${s2};`;
    }), "");
  }
  update(t3, [e5]) {
    const { style: r4 } = t3.element;
    if (void 0 === this.ft) return this.ft = new Set(Object.keys(e5)), this.render(e5);
    for (const t4 of this.ft) null == e5[t4] && (this.ft.delete(t4), t4.includes("-") ? r4.removeProperty(t4) : r4[t4] = null);
    for (const t4 in e5) {
      const s2 = e5[t4];
      if (null != s2) {
        this.ft.add(t4);
        const e6 = "string" == typeof s2 && s2.endsWith(ie);
        t4.includes("-") || e6 ? r4.setProperty(t4, e6 ? s2.slice(0, -11) : s2, e6 ? ee : "") : r4[t4] = s2;
      }
    }
    return R;
  }
...
// js/legend.ts
function zip(a2, b2) {
  const maxLength = Math.min(a2.length, b2.length);
  return a2.slice(0, maxLength).map((el, i4) => [el, b2[i4]]);
}
var Legend = class extends LitWidget {
  constructor() {
    super(...arguments);
    this.title = "";
    this.legendKeys = [];
  ...
  renderLegend(title) {
    const legend = zip(this.legendKeys, this.legendColors).map(
      ([key, color]) => ke`<li>
                    <span style="${se({ background: color })}">
                    </span>${key}
                </li>`
    );
    return ke`<div class="legend">
                        ${title ? ke`<h4 class="legend-title">${title}</h4>` : D}
...
__decorateClass([
  n4({ type: String })
], Legend.prototype, "title", 2);
__decorateClass([
  n4({ type: Array })
], Legend.prototype, "legendKeys", 2);
__decorateClass([
  n4({ type: Array })
], Legend.prototype, "legendColors", 2);
__decorateClass([
  n4({ type: Boolean })
], Legend.prototype, "addHeader", 2);
__decorateClass([
  n4({ type: Boolean })
], Legend.prototype, "showCloseButton", 2);
